...
export default function Hero3D() {
  ...
  return (
    <section style={{
      /* KEY FIX: mobile = auto height, desktop = 100vh */
      ...
      <div style={{
        ...
        /* mobile: full screen fixed bg */
        height: isMobile ? '100vh' : '100%',
      }}>
        <Canvas camera={{position:[0,0,8],fov:70}} style={{background:'transparent'}}
          dpr={[1, isMobile ? 1 : 1.5]} performance={{min:0.5}}>
          <Scene/>
        </Canvas>
      </div>

      {/* Glow orbs */}
      <div style={{position:'absolute',top:'10%',left:'5%',width:280,height:280,background:'radial-gradient(circle,rgba(124,58,237,0.1),transparent 70%)',borderRadius:'50%',pointerEvents:'none',zIndex:1,animation:'glowPulse 4s ease-in-out infinite'}}/>
      <div style={{position:'absolute',bottom:'10%',right:'5%',width:200,height:200,background:'radial-gradient(circle,rgba(219,39,119,0.08),transparent 70%)',borderRadius:'50%',pointerEvents:'none',zIndex:1,animation:'glowPulse 5s ease-in-out infinite reverse'}}/>

      {/* TEXT CONTENT */}
      <div style={{
        position:'relative', zIndex:2,
        /* KEY FIX: mobile padding tight, no min-height */
        minHeight: isMobile ? 'auto' : '100vh',
        display:'flex', alignItems:'center',
        padding: isMobile ? '62px 20px 16px' : '0 80px',
        ...
        <motion.div variants={cV} initial="hidden" animate="visible"
          ...
          <motion.div variants={iV} style={{
            ...
            borderRadius:50, padding:'7px 16px', marginBottom:14, backdropFilter:'blur(10px)',
          }}>
            ...
            <span style={{color:'#a78bfa',fontWeight:700,fontSize:'0.8rem',fontFamily:"'Inter',sans-serif"}}>
          ...
          <motion.h1 variants={iV} style={{
            fontFamily:"'Inter',sans-serif",
            fontSize: isMobile ? 'clamp(1.6rem,6.5vw,2rem)' : 'clamp(1.8rem,3vw,3rem)',
            fontWeight:900, lineHeight:1.1, marginBottom:12, letterSpacing:'-0.5px',
          }}>
          ...
          {/* Subtitle */}
          <motion.p variants={iV} style={{
            color:'rgba(255,255,255,0.45)', fontSize:'0.84rem', lineHeight:1.75,
            maxWidth: isMobile ? '100%' : 420,
            margin: isMobile ? '0 auto 18px' : '0 0 22px',
            fontFamily:"'Inter',sans-serif",
          }}>
          ...
          <motion.div variants={iV} style={{
            ...
            <motion.button onClick={handleDownload}
              ...
              style={{background:'linear-gradient(135deg,#7c3aed,#db2777)',color:'white',
                padding:'10px 22px',borderRadius:11,fontWeight:700,fontSize:'0.83rem',
            ...
            <motion.a href="#projects" whileHover={{scale:1.04,y:-3}} whileTap={{scale:0.97}}
              style={{background:'rgba(255,255,255,0.05)',backdropFilter:'blur(20px)',
                color:'white',padding:'10px 22px',borderRadius:11,fontWeight:700,
            ...
            <motion.a href="#contact" whileHover={{scale:1.04,y:-3}} whileTap={{scale:0.97}}
              style={{background:'rgba(219,39,119,0.1)',backdropFilter:'blur(20px)',
                color:'#f472b6',padding:'10px 22px',borderRadius:11,fontWeight:700,
                fontSize:'0.83rem',textDecoration:'none',
                display:'inline-flex',alignItems:'center',gap:6,
          ...
          <motion.div variants={iV} style={{
            ...
            {[
              ...
              <motion.div key={i} whileHover={{y:-3,scale:1.04}} style={{textAlign:'center',cursor:'default'}}>
                <motion.div style={{
                  fontFamily:"'Inter',sans-serif",
                ...
                <div style={{color:'rgba(255,255,255,0.35)',fontSize:'0.6rem',fontWeight:600,
                  marginTop:2,letterSpacing:'1px',textTransform:'uppercase',fontFamily:"'Inter',sans-serif"}}>
                  {label}
                </div>
              </motion.div>
            ))}
          ...
          {/* Scroll — inside content on mobile */}
          {isMobile && (
            <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:2}}
              style={{display:'flex',flexDirection:'column',alignItems:'center',gap:5,marginTop:20}}>
              <span style={{color:'rgba(255,255,255,0.25)',fontSize:'0.58rem',fontWeight:700,
              ...
              <motion.div animate={{y:[0,7,0]}} transition={{duration:1.5,repeat:Infinity}}
                style={{width:20,height:32,border:'1.5px solid rgba(124,58,237,0.28)',
                ...
                <motion.div animate={{y:[0,9,0],opacity:[1,0,1]}} transition={{duration:1.5,repeat:Infinity}}
                  style={{width:2,height:5,background:'linear-gradient(to bottom,#7c3aed,#db2777)',borderRadius:2}}/>
              </motion.div>
            </motion.div>
          )}
        </motion.div>
        ...
        <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:2.2}}
          ...
          <motion.span animate={{opacity:[0.3,0.8,0.3]}} transition={{duration:2,repeat:Infinity}}
            style={{color:'rgba(255,255,255,0.28)',fontSize:'0.6rem',fontWeight:700,
              letterSpacing:'4px',textTransform:'uppercase',fontFamily:"'Inter',sans-serif"}}>Scroll</motion.span>
