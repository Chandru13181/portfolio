...
function Scene({ isMobile }) {
  return (
    <>
      ...
      <Stars radius={200} depth={80} count={isMobile ? 2000 : 5000} factor={5} saturation={0.3} fade speed={0.4}/>
      <FullScreenDots/>
      <ConnectionLines/>
      {!isMobile && <MainOrb/>}
      {!isMobile && <OrbitRings/>}
      ...
      {!isMobile && <FloatingShape position={[7,-2,-5]}  color="#db2777" shape="sphere" speed={0.9} scale={0.55}/>}
    </>
  )
}
...
export default function Hero3D() {
  ...
  const iV={hidden:{opacity:0,y:20},visible:{opacity:1,y:0,transition:{duration:0.6,type:'spring',stiffness:80}}}
  ...
  return (
    <section style={{
      ...
      {/* 3D Canvas — always absolute */}
      <div style={{position:'absolute',inset:0,zIndex:0,pointerEvents:'none'}}>
        <Canvas
          camera={{position:[0,0,8],fov:isMobile?85:70}}
      ...
      {/* Glow orbs */}
      <div style={{position:'absolute',top:'8%',left:'3%',width:250,height:250,background:'radial-gradient(circle,rgba(124,58,237,0.1),transparent 70%)',borderRadius:'50%',pointerEvents:'none',zIndex:1}}/>
      <div style={{position:'absolute',bottom:'8%',right:'3%',width:180,height:180,background:'radial-gradient(circle,rgba(219,39,119,0.08),transparent 70%)',borderRadius:'50%',pointerEvents:'none',zIndex:1}}/>
      ...
      <div style={{
        ...
        display:'flex',
        alignItems:'center',
        /* KEY: padding-top accounts for navbar height */
        padding: isMobile
          ? '68px 24px 24px'   /* mobile: navbar=58px + 10px gap */
          : '0 80px',
        ...
        <motion.div
          ...
          <motion.div variants={iV} style={{
            ...
            borderRadius:50, padding:'7px 16px', marginBottom:isMobile?12:18,
          ...
          <motion.h1 variants={iV} style={{
            fontFamily:"'Inter',sans-serif",
            fontSize: isMobile ? 'clamp(1.5rem,6vw,1.9rem)' : 'clamp(1.8rem,3vw,3rem)',
      ...
      <motion.div
        ...
        style={{
          ...
          gap:5, paddingBottom: isMobile ? 16 : 24,
        }}>
