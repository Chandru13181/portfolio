...
/* Consistent heading style across footer */
const ColTitle = ({ children }) => (
  <h4 style={{
    fontFamily:"'Syne',sans-serif", fontSize:12,
    color:"#ec4899", letterSpacing:2,
...
const Footer = () => (
  <footer style={{ position:"relative", zIndex:1, borderTop:"1px solid rgba(124,58,237,.15)", background:"rgba(8,0,16,.97)" }}>
    ...
    <div style={{ borderBottom:"1px solid rgba(124,58,237,.1)" }}>
      <div className="container" style={{ padding:"44px 48px", textAlign:"center" }}>
        <div style={{ fontFamily:"'Inter',sans-serif", fontSize:11, color:"#a855f7", letterSpacing:3, textTransform:"uppercase", marginBottom:12, fontWeight:600 }}>
          ✦ AVAILABLE FOR WORK
        </div>
        <h3 style={{ fontFamily:"'Syne',sans-serif", fontSize:"clamp(22px,3vw,36px)", fontWeight:800, color:"#fff", lineHeight:1.25, marginBottom:24 }}>
          Got a project idea?{" "}
        ...
        <div style={{ display:"flex", gap:14, justifyContent:"center", flexWrap:"wrap" }}>
          <motion.a href="#contact" onClick={e=>go(e,"contact")} whileHover={{ scale:1.04 }}
            style={{ padding:"12px 28px", borderRadius:50, background:"linear-gradient(135deg,#7c3aed,#ec4899)", color:"#fff", fontFamily:"'Syne',sans-serif", fontSize:14, fontWeight:700, textDecoration:"none", display:"inline-flex", alignItems:"center", gap:8 }}>
            💬 Start a Project
          </motion.a>
          <motion.a href="/resume.pdf" download whileHover={{ scale:1.04 }}
            style={{ padding:"11px 28px", borderRadius:50, background:"rgba(255,255,255,.06)", border:"1px solid rgba(255,255,255,.2)", color:"#fff", fontFamily:"'Inter',sans-serif", fontSize:14, textDecoration:"none", display:"inline-flex", alignItems:"center", gap:8 }}>
            ↓ Download CV
          </motion.a>
        </div>
      </div>
    ...
    {/* ── MAIN GRID ── */}
    <div className="container" style={{ padding:"52px 48px 0" }}>
      <div style={{ display:"grid", gridTemplateColumns:"1.4fr 1fr 1fr 1fr", gap:40 }}>
        ...
        <div>
          <div style={{ fontFamily:"'Syne',sans-serif", fontSize:18, fontWeight:800, letterSpacing:2, color:"#fff", marginBottom:12, display:"flex", alignItems:"center", gap:9 }}>
            <div style={{ width:30, height:30, borderRadius:7, background:"linear-gradient(135deg,#7c3aed,#ec4899)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
          ...
          <p style={{ fontFamily:"'Inter',sans-serif", fontSize:13, color:"#c4b5d4", lineHeight:1.8, marginBottom:20 }}>
          ...
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            {SOCIALS.map(({ Icon, href, label, bg, bd }) => (
              <motion.a key={label} href={href} target="_blank" rel="noopener noreferrer"
                title={label} aria-label={label} whileHover={{ y:-3, scale:1.1 }}
                style={{ width:34, height:34, borderRadius:8, background:bg, border:`1px solid ${bd}`, display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", textDecoration:"none" }}>
                <Icon/>
              </motion.a>
            ))}
          </div>
            ...
            {NAV.map(l => (
              <li key={l}>
                <motion.a href={`#${l.toLowerCase()}`} onClick={e=>go(e,l.toLowerCase())}
                  whileHover={{ x:4 }}
                  style={{ fontFamily:"'Inter',sans-serif", fontSize:13, color:"#c4b5d4", textDecoration:"none", display:"flex", alignItems:"center", gap:8, transition:"color .2s" }}
        ...
        <div>
          ...
          <ul style={{ listStyle:"none", padding:0, display:"flex", flexDirection:"column", gap:10 }}>
            {SERVICES.map(s => (
              <li key={s} style={{ fontFamily:"'Inter',sans-serif", fontSize:13, color:"#c4b5d4", display:"flex", alignItems:"center", gap:8 }}>
        ...
        <div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {[
              ...
              <div key={val} style={{ display:"flex", alignItems:"flex-start", gap:8, fontFamily:"'Inter',sans-serif", fontSize:12, color:"#c4b5d4", lineHeight:1.5 }}>
                <span style={{ fontSize:13, flexShrink:0, marginTop:1 }}>{icon}</span>{val}
              </div>
            ))}
          </div>
        </div>
    ...
    {/* ── BOTTOM BAR ── */}
    <div className="container" style={{ padding:"18px 48px", borderTop:"1px solid rgba(124,58,237,.1)", marginTop:48, display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:10 }}>
      <p style={{ fontFamily:"'Inter',sans-serif", fontSize:12, color:"#6b5a7e", display:"flex", alignItems:"center", gap:5 }}>
        © {new Date().getFullYear()} Chandru M — Made with{" "}
        <svg width="11" height="11" viewBox="0 0 24 24" fill="#ec4899"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg>
        in Chennai
      </p>
      <p style={{ fontFamily:"'Inter',sans-serif", fontSize:12, color:"#6b5a7e" }}>
        Chandru M UI UX Designer & Web Developer
      </p>
      <motion.button onClick={()=>window.scrollTo({top:0,behavior:"smooth"})}
        whileHover={{ scale:1.05, color:"#a855f7", borderColor:"#a855f7" }}
        style={{ display:"flex", alignItems:"center", gap:5, padding:"6px 16px", border:"1px solid rgba(124,58,237,.25)", color:"#c4b5d4", fontFamily:"'Inter',sans-serif", fontSize:12, cursor:"pointer", borderRadius:50, background:"transparent", transition:"all .2s" }}>
    ...
    <style>{`
      @media(max-width:900px){ footer .container>div:nth-child(2){ grid-template-columns:1fr 1fr!important } }
      @media(max-width:560px){ footer .container>div:nth-child(2){ grid-template-columns:1fr!important } }
      @media(max-width:640px){
        footer .container:first-of-type>div{ flex-direction:column!important; align-items:center!important; text-align:center!important }
      }
    `}</style>
  </footer>
)
